...
function toDraft(g: SiteGeofence): Draft {
  return {
...
export default function Settings() {
  const [saved, setSaved] = useState<SiteGeofences | null>(null);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    setError(null);
    try {
      setSaved(await api.geofences());
    } catch (err) {
  ...
  const patch = (kind: GeofenceKind, next: SiteGeofence) =>
    setSaved((s) => (s ? { ...s, [kind]: next } : s));

  return (
    <div className="stack animate-fade-up" style={{ gap: 20 }}>
      <PageHeader
        title="Geofences"
        description="Two circles, doing two different jobs. Attendance is measured against the hostel one and never blocked by it; a resident recording their own meal is refused outside the mess one."
      />

      <GeofenceForm
        kind="hostel"
        saved={saved.hostel}
        onSaved={(next) => patch("hostel", next)}
        title="Hostel"
        description="The property boundary. Marking attendance from outside it is flagged for review, never blocked."
        labelPlaceholder="Thapar, Block B"
      />

      <GeofenceForm
        kind="mess"
        saved={saved.mess}
        onSaved={(next) => patch("mess", next)}
        title="Mess"
        description={`The servery itself. A resident recording their own meal by face or fingerprint is refused outside this circle, so it wants to be tight — around ${SUGGESTED_MESS_RADIUS_METRES} m, not the hostel's radius. The counter's QR scan is never blocked by it.`}
        labelPlaceholder="Thapar, Mess"
      />
...
interface GeofenceFormProps {
  kind: GeofenceKind;
  saved: SiteGeofence;
  onSaved: (next: SiteGeofence) => void;
  title: string;
  description: string;
...
function GeofenceForm({
  ...
  const submit = async (e: FormEvent) => {
    ...
    setFormError(null);
    try {
      const next = await api.saveGeofence(kind, {
        latitude,
        longitude,
        radiusMetres,
      ...
      toast.show(`${title} geofence updated`, "success");
    } catch (err) {
  ...
  // Ids have to be unique across both forms on the page, or every label points
  // at the hostel input.
    ...
    <form className="card stack" onSubmit={submit}>
      ...
      {!saved.configured && (
        <p className="error-text" role="status">
          No mess fence has been set, so it's currently borrowing the hostel
          circle above — {saved.radiusMetres} m around {saved.locationLabel}.
          That's almost certainly wrong in one direction or the other: too wide
          and a resident can record a meal from their room, too far off-centre
          and nobody standing in the servery can record one at all. Set the real
          coordinates here.
        </p>
      )}
      ...
      <div className="form-grid">
        ...
        <div className="field">
          <label htmlFor={id("radius")}>Radius in metres</label>
          <input
            id={id("radius")}
            inputMode="numeric"
          ...
          <p className="caption">
            ...
            {kind === "mess" && ` · around ${SUGGESTED_MESS_RADIUS_METRES} suits one building`}
          </p>
        </div>

        <div className="field">
          <label htmlFor={id("label")}>Location name</label>
          <input
            id={id("label")}
          ...
          />
          <p className="caption">What a resident sees on their own record.</p>
        </div>
      </div>
